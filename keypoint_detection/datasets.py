...
class ImgSequenceDatasetGen(Dataset):
    ...
    def __getitem__(self, idx):
        seq_data_dict = self._process_data(
            self.filename, self.num_timesteps, self.shuffle)
    ...
    def _process_data(self, filename, num_timesteps, shuffle=True):
        ...
        for k in seq_data_dict.keys():
            if k == 'image':
                seq_data_dict[k] = torch.from_numpy(
                    seq_data_dict[k]).permute(
                    0, 1, 4, 2, 3)
            else:
...
class ImgSequenceDataset(Dataset):
    def __init__(self, filename, num_timesteps, shuffle=True):
        self.filename = filename
        self.num_timesteps = num_timesteps
        self.seq_data_dict = self._process_data(
            filename, num_timesteps, shuffle)
...
def get_dataset(data_dir,
                ...
                num_workers=2,
                seed=0):
    # Find files for dataset. Each file contains a sequence of arbitrary
    # length:
    ...
    shapes = {k: (None,) + v.shape[1:]
              for k, v in iter(dataloader).next().items()}
...
def get_sequence_dataset(data_dir,
    ...
    dsets = [
        ImgSequenceDataset(
            fname,
            num_timesteps,
            shuffle=shuffle) for fname in filenames]
    dataset = ConcatDataset(dsets)
...
def _read_numpy_sequences(filename):
    ...
    # sequence_dict['file_idx'] = np.full(num_frames, os.path.basename(filename))
    # why basename?
...
def _format_image_data(image):
    ...
    if not np.issubdtype(image.dtype, np.uint8):
        raise ValueError(
            'Expected image to be of type {}, but got type {}.'.format(
