...
class KeypointModel(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.images_to_keypoints_net = ImagesToKeypEncoder(
            cfg, input_shape_no_batch)
        self.keypoints_to_images_net = KeypToImagesDecoder(
            cfg, input_shape_no_batch)
    ...
    def forward(self, img_seq):
        ...
        reconstructed_img_seq = self.keypoints_to_images_net(
            keypoints_seq, img_seq[:, 0, :, :, :], keypoints_seq[:, 0, :, :])
    ...
    def step(self, batch, batch_idx, is_train=True):
        ...
        keypoints_seq, heatmaps_seq, reconstructed_img_seq = self.forward(
            img_seq)

        reconstruction_loss = F.mse_loss(
            img_seq, reconstructed_img_seq, reduction='sum')
    ...
    def aggregate_metrics(self, outputs, is_train=True):
        ...
        avg_recon_loss = torch.stack(
            [x[pfx + 'recon_loss'] for x in outputs]).mean()
        avg_hmap_loss = torch.stack([x[pfx + 'hmap_loss']
                                     for x in outputs]).mean()
        avg_temporal_loss = torch.stack(
            [x[pfx + 'temporal_loss'] for x in outputs]).mean()
