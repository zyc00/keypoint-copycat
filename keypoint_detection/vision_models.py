...
class ImageEncoder(nn.Module):
        ...
        if np.log2(input_shape[1] / output_map_width) % 1:
            raise ValueError(
                ...
                'square, but got {} and {} with ratio {}'.format(
                    input_shape[1],
                    output_map_width,
                    input_shape[1] / output_map_width))

        C, H, W = input_shape

        layers = []
        layers.extend([nn.Conv2d(C, initial_num_filters,
                                 kernel_size=3, padding=1), nn.LeakyReLU(0.2)])

        width = W
        num_filters = initial_num_filters
        while width > output_map_width:
            # Reduce resolution:
            layers.extend([nn.Conv2d(num_filters,
                                     2 * num_filters,
                                     stride=2,
                                     kernel_size=3,
                                     padding=1),
    ...
    def forward(self, x):
        ...
        x = self.encoder(x)
        if self.debug:
            print("Encoded Image shape: ", x.shape)
...
class ImageDecoder(nn.Module):
    def __init__(self, input_shape, output_width,
        ...
        for i in range(num_levels):
            ...
            c_in = num_filters
            for _ in range(layers_per_scale):
                layers.extend([nn.Conv2d(c_in, num_filters //
                                         2, kernel_size=3, padding=1), nn.LeakyReLU(0.2)])
                c_in = c_in // 2
