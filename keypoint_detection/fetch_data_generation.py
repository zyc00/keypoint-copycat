...
def get_frame(env, crop=(50, 350), size=(64, 64)):
    ...
    if crop:
        frame = frame[crop[0]:crop[1], crop[0]:crop[1]]
...
def main(args):
    ...
    # np.savez_compressed(save_path, acs=actions, obs=observations,
    # info=infos, frames=frames) # save the file
...
def goToGoal(env, lastObs):
    ...
    if not args.display:
        episodeFrames.append(get_frame(env))
    else:
        env.render()

    while np.linalg.norm(
            object_oriented_goal) >= 0.005 and timeStep <= env._max_episode_steps:
        # env.render()
        action = [0, 0, 0, 0]
        object_oriented_goal = object_rel_pos.copy()
        object_oriented_goal[2] += 0.03
    ...
    while np.linalg.norm(
            object_rel_pos) >= 0.005 and timeStep <= env._max_episode_steps:
        # env.render()
        action = [0, 0, 0, 0]
    ...
    while np.linalg.norm(
            goal - objectPos) >= 0.01 and timeStep <= env._max_episode_steps:
        # env.render()
        action = [0, 0, 0, 0]
